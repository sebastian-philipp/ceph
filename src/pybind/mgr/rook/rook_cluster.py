...
from .rook_ceph_client import cephnfs as cnfs
from .rook_ceph_client import cephobjectstore as cos
from .rook_ceph_client import cephcluster as ccl
            ...
            if drive_group.hosts(all_hosts)[0] not in [n.name for n in current_nodes]:
                ...
                if block_devices:
                    pd.devices = ccl.DevicesList(
                        ccl.DevicesItem(name=d) for d in block_devices
                    )
                if directories:
                    ...
                    if current_node.name == drive_group.hosts(all_hosts)[0]:
                        if block_devices:
                            ...
                            current_node.devices.extend(
                                ccl.DevicesItem(name=n) for n in new_devices
                            )
